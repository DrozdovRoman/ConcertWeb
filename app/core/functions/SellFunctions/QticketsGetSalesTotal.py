...
# Входные данные QticketsGetSalesTotal:
...
# Выходные данные QticketsGetSalesTotal:
...
def QticketsGetSalesTotal(accountSessionDict, qticketsAccountData):
    resultData = {}
    for accountName in accountSessionDict.keys(): # Проходимся по всем элементам словаря сессий
        ...
        if (accountIds != None):
            for concertInfo in accountIds:
                link = "https://qtickets.app/stats/show/" + concertInfo["sellId"]
...
def QticketsGetSalesTotalTicket(pageSoup):
    result = {
        "totalTicket" : 0,
        "refundTicket" : 0,
...
def QticketsGetSalesTotalSum(pageSoup):
    result = {
        "totalSum" : 0,
        "refundSum" : 0,
...
def getSalesInformation(pageSoup):
    dictTicketInformation = QticketsGetSalesTotalTicket(pageSoup)
    dictSumInformation = QticketsGetSalesTotalSum(pageSoup)
